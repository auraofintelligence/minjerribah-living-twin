...
This is a plain-language licence for minjerribah-living-twin.
...
Based on minjerribah-living-twin by Luke Nathan Hayes / Strange But True / Aura of Intelligence.
Original project: https://github.com/auraofintelligence/minjerribah-living-twin
...
**Babylon.js.** `vendor/babylon.js`, `vendor/babylon.gui.js` and `vendor/babylon.materials.js` are
Babylon.js, vendored so this project runs offline with no build step and no CDN. They are not covered
by this licence and remain under their own, which is Apache 2.0 and is included at
`vendor/BABYLON-LICENSE.md`.

**Quandamooka cultural material.** There is none in this repository, and that is deliberate rather
than accidental. Minjerribah is Quandamooka Country. QYAC, the Quandamooka Yoolooburrabee Aboriginal
Corporation, holds and manages the Quandamooka People's native title rights and interests, and QYAC has
not seen this project. `data/lore.json` carries the prohibitions this build is checked against, and
`tools/gate-audit.mjs` enforces them on every pack. `docs/CULTURAL-REVIEW.md` is a queue of open
questions for the Traditional Owners: nothing in it is approved, and nothing in it should be read as
consent. No permission granted by this licence extends to Quandamooka cultural knowledge, language,
ceremony or places, because that was never the author's to license.
